...
(function () {
  ...
  function buildClothesline() {
    ...
    const baseSize = isMobile
      ...
      : 190;
    const rowHeight = baseSize + (isMobile ? 120 : 140);

    // Generous bottom padding guarantees the Continue button below can
    // never end up overlapped by the last row's photos.
    clothesline.style.height = rows * rowHeight + 90 + "px";
    ...
    for (let r = 0; r < rows; r++) {
      ...
      for (let k = 0; k < rowCount; k++) {
        ...
        const dangle = rand(16, Math.max(20, rowHeight - size - 80));
        const tilt = rand(-6, 6);
